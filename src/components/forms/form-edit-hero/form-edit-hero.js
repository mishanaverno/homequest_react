...
export default class FormEditHero extends Form{
    state = {
        inputs: [
            {
                name: "login",
    ...
    validate = () => {
        ...
        //pass
        if(validator.minLength(password, 1) && !validator.minLength(password, 6)){
            valid = false;
            errors.password = 'Length should be more than 5';
        }
        if(validator.minLength(password, 1) && !validator.maxLength(password, 50)){
            valid = false;
            errors.password = 'Length should be less than 50';
        }
        //c_pass
